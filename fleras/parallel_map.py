...
def build_dataflow(
    ...
    ds = parallel_map_as_tf_dataset(
        ...
        roundrobin_sizes=roundrobin_sizes, disable_parallelity=disable_parallelity)

    return ds.ragged_batch(batch_size, drop_remainder=is_training, row_splits_dtype=tf.int32)

    # return ds.batch(batch_size, drop_remainder=is_training)


def to_ragged_spec(tensor_spec, ragged_rank=1):
    return tf.TensorSpec(shape=(None,) + tensor_spec.shape[1:], dtype=tensor_spec.dtype)


def to_sparse_spec(tensor_spec):
    return tf.SparseTensorSpec(shape=tensor_spec.shape, dtype=tensor_spec.dtype)
...
def parallel_map_as_tf_dataset(
    ...
    if isinstance(output_signature, dict):
        output_signature = {
            key:
                to_ragged_spec(tensor_spec) if key.startswith("ragged_") else
                to_sparse_spec(tensor_spec) if key.startswith('sparse_') else
                tensor_spec
            for key, tensor_spec in output_signature.items()
        }
    ...
    def process_dict(d):
        return {
            key.replace("ragged_", "") if key.startswith("ragged") else
            key.replace("sparse_", "") if key.startswith("sparse") else
            key: value
            for key, value in d.items()
        }

    ds = ds.map(process_dict)
...
def advance_rng(rng, num_generated_ints):
    for _ in range(num_generated_ints):
